...
public class AlbumImpl implements Album {
    ...
    public Photo getPhotoFromAlbum(int photoId, int albumId) {
        Photo pattern = new Photo(albumId, photoId, null, null, null);//определили тот обьект с которым сравнивали
        for (int i = 0; i < size; i++) {
    ...
    @Override
    public Photo[] getAllPhotosAlbum(int albumId) {
...
//        to = to >= 0 ? to : -to - 1;
        return Arrays.copyOfRange(photos, from, to); // Range - диапазон, создаем новый массив с нужными фот
