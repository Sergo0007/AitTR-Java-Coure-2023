...
public class LibAppl {
    public static void main(String[] args) {
        ...
        System.out.println("---------Mailing List All User---------");
        ...
        for (Reader reader : library.getReaders()) {
            if (reader.isSubscriber()) {
                if (reader.getBooks().size() > 2) {
                    if (!result.containsKey("TOO_MUCH")) {//по этому ключу нет еще значений
                        result.put("TOO_MUCH", new ArrayList<>());
                    }
                    result.get("TOO_MUCH").add(new Email(reader.getEmail()));
                } else {
                    if (!result.containsKey("OK")) {////по этому ключу нет еще значений
